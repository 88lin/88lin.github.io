...
新增 `/2/system/`「88lin 的白板｜开源项目、博客与工具」，从首页与关于页的“画布”导航进入。九张内容卡片与两张贴纸存于静态 HTML；按反馈删除了预置咖啡贴纸与可见灵感来源文字。无 JavaScript 时仍可阅读内容卡片和访问真实链接。

- 独立标题、摘要、canonical、Open Graph / Twitter 信息，以及最新导航和交互修正后的真实截图生成的 `assets/og/profile-2-system.jpg`（1200 × 630）。
- 生成脚本添加 WebPage 节点，与现有 WebSite、Person 使用同一身份；`hasPart` 对应十一个实际存在的预置卡片锚点。访客创作仅存在本地，不写入公共结构化数据或内容索引。
- 根 sitemap、根 `llms.txt` 与个人主页 `llms.txt` 已加入白板。没有公共留言和服务端照片上传；本地照片仅用于当前访客自己的白板。
- Chromium 实测卡片拖动、旋转、调整大小、涂鸦与擦除、撤销重做、本地持久化、备份导入与无效文件保护、PNG 导出、全屏、真实双指缩放、无脚本阅读及存储失败提示。
- 最新一轮对首页、关于页和画布检查 320、360、390、430、640、768、1024、1440 px，导航无变形和横向溢出，画布工具栏无溢出；115 项检查覆盖鼠标、键盘与模拟触摸、搜索焦点与空态、侧栏恢复焦点、编辑提示、缩放和无脚本阅读，无页面脚本错误或 HTTP 资源错误。模拟触摸未覆盖真机 Safari。
- 已核对 93 个本地链接 / 资源引用、3 页元数据与 JSON-LD、11 个预置锚点、sitemap / llms、分享图尺寸与 SEO 生成器幂等性；上线效果需要发布后再核对。

以下保留前一轮首页、关于页与 404 页的审计记录。

2026-09-21 内容修整：画布标题、描述和卡片名称按实际内容更新。项目卡片对应三个真实仓库，博客精选卡片对应博客文章 20、notion、34 和博客书单 /book，主页截图直标个人主页；实用教程链接文章 14、10、50。目录、JSON-LD `hasPart`、两级 `llms.txt` 和分享截图随之同步，保留 11 个稳定卡片锚点。内容依据为本站首页、关于页与用户提供的博客书单链接，本轮没有重新在线核对仓库数据。26 项针对性浏览器检查、105 个本地引用与 SEO 生成器幂等性检查通过。
...
日期应替换为内容实际修改或核对日期。脚本只使用 Python 标准库，生成首页、关于页和白板三个页面的 JSON-LD、项目片段 ID 和 `2/llms.txt`；访问者无需运行脚本，发布时直接使用生成后的静态文件。
...
1. 检查 `/2/`、`/2/about/`、`/2/system/` 返回更新后的标题、canonical 和 JSON-LD；旧 `index.html` 地址继续正确跳转。
2. 确认根目录 `robots.txt`、`sitemap.xml`、`llms.txt` 与 `/2/llms.txt` 一并发布。
3. 在已有 Google Search Console / Bing Webmaster Tools 站点中检查三个正式网址。若根 sitemap 已提交，无需创建重复站点地图；可按需请求重新抓取。
